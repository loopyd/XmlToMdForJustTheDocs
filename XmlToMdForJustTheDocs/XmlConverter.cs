...
internal static class XmlToMarkdown
{
    ...
    private static readonly Func<string, XElement, string[]> d = (att, node) => new[]
    {
        node.Attribute(att).Value,
        node.Nodes().ToMarkDown()
    };

    private static readonly Dictionary<string, Func<XElement, IEnumerable<string>>> methods = new()
    {
        {
            "doc", x => new[]
            {
